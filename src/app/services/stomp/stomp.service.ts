...
export class STOMPService {
    ...
    public configure( config?: StompConfig ): void {
        ...
        this.client = Stomp.client( `${scheme}://${this.config.host}:${this.config.port}` );
    ...
    public publish( message?: string, correlationId?: string ): void {
        this.client.send( this.config.queue, { "correlation-id": correlationId }, message );
    }
    ...
    public subscribe(): Object {
        ...
        var selector = "JMSCorrelationID = '" + this.config.correlationId + "'";

        // Subscribe to our configured queue
        subscription = this.client.subscribe( this.config.queue, this.on_message, { ack: 'auto', 'selector': this.config.selector } );
    ...
    public on_connect = ( correlationId ) => {
